...
def staff_home_page
  ...
  if staff_options == '1'
    ...
    overall_data = {}
    banking_details = {
      'Account name': acct_name, 'Opening Balance': opening_bal.to_i,
...
def create_customer_pin
  ...
  if data.key?(customer_acct)
    ...
    if acct_pin.to_i.to_s == acct_pin && acct_pin.length >= 4 && acct_pin.length <= 6
      ...
      # checking if customer_login.txt is empty, the essence of this is to enable us have access to file through the JSON module.
  ...
  if data.key?(customer_acct) && data[customer_acct]['Account pin'] == acct_pin
   ...
   while login == true
    ...
    if customer_login_options == '1'
     ...
     puts("\n\tYour account balance is: " + customer_details[customer_acct]['Opening Balance'].to_s)
     ...
     if deposit.to_i.to_s == deposit
      ...
      File.open('customer.txt') do |f_obj|
       overall_data = JSON.load(f_obj)
       # Then append the present user details
       overall_data[customer_acct]['Opening Balance'] += deposit.to_i
      end
      File.open('customer.txt', 'w') do |fil|
       JSON.dump(overall_data, fil)
      end
     ...
     if withdrawal.to_i.to_s == withdrawal
      ...
      File.open('customer.txt') do |f|
       ...
       # Then append the present user details
       overall_data[customer_acct]['Opening Balance'] -= withdrawal.to_i
      end
